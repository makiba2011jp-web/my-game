...
function interactNPC(n) {
  ...
  if (n.wail) { talkWail(); return; }                          // 嘆きの亡霊: 英語学習の苦悩をランダムに叫ぶ(AIなし)
...
// 嘆きの亡霊: 話しかけるとランダムな苦悩のセリフ(AI会話ではない・こちらから話題は選べない)
function talkWail() {
  for (const k in keys) keys[k] = false;
  const line = WAIL_LINES[Math.floor(rnd() * WAIL_LINES.length)];
