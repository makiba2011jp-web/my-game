...
// 装備＋永続バフ(好感度100)の合計を反映して player.atk/def/maxhp を再計算
function recomputeStats() {
  ...
  const pb = permaTotals();
  player.atk = player.baseAtk + a + pb.atk;
  player.def = player.baseDef + d + pb.def;
  player.maxhp = player.baseMaxhp + h + pb.hp;
...
// 嘆きの亡霊(各エリアに配置)。話しかけると英語学習の苦悩をランダムに叫ぶ。AI会話ではない。
// area = "dungeon"/"dungeon2"/"fire"/"tower"/"castle"。エリアごとに別々のセリフ集(各10パターン)。
function WAIL_NPC(tx, ty, area) { return { id: "wail_" + area, name: "嘆きの亡霊", tx, ty, color: "#9a90b8", wail: true, wailArea: area }; }
// エリア別・嘆きの亡霊のセリフ集(各10パターン・各要素=表示する行の配列)
const WAIL_LINES = {
  // 古代の遺跡: 発音の悩み
  dungeon: [
    ["L と R の違いなんて 日本人には無理だ…！", "right も light も 同じに聞こえる…！"],
    ["th の音…！ 舌を噛むなんて できるか…！"],
    ["発音記号が 呪文にしか 見えない…！"],
    ["V と B の口の形…！ もう区別がつかない…！"],
    ["「water」が どうしても「ワラ」に聞こえる…！"],
    ["アクセントの位置を まちがえると 全然 通じない…！"],
    ["silent letter… 読まない文字なんて 意地悪だ…！"],
    ["「th」を 練習しすぎて 舌が つった…！"],
    ["カタカナ英語が 邪魔をする…！ 直らない…！"],
    ["「r」で 舌を巻く…？ 一生 できる気がしない…！"],
  ],
  // 氷の遺跡: 冠詞・文法の悩み
  dungeon2: [
    ["a と the、どちらを使うか 地獄だ…！", "日本語には ないんだ…！"],
    ["三単現の s… つけ忘れて いつも 減点される…！"],
    ["可算名詞と不可算名詞…！ water に s は つかない…！？"],
    ["仮定法… if I were…！ なぜ was じゃ ないんだ…！"],
    ["時制の一致…！ どこまで 過去にすれば いいんだ…！"],
    ["関係代名詞 which と that の使い分けが 凍りつく…！"],
    ["受動態と能動態…！ 主語が 入れ替わって 混乱する…！"],
    ["複数形が 不規則すぎる…！ child が children…！？"],
    ["前置詞… in? on? at? もう分からない…！"],
    ["「the」を つけるか つけないか… 一生 迷い続ける…！"],
  ],
  // 炎の遺跡: 単語・暗記の悩み
  fire: [
    ["単語を100回書いても 翌朝には 忘れている…！"],
    ["熟語が 多すぎる…！ look up? look after? 覚えきれない…！"],
    ["同じ意味の単語が いくつあるんだ…！ big, large, huge…！"],
    ["スペルと発音が 一致しない…！ knife の k は どこへ…！"],
    ["接頭辞に 接尾辞…！ 単語が 燃えるように 増えていく…！"],
    ["覚えた単語が 実戦で 一つも 出てこない…！"],
    ["「-ough」の読み方が 6通り…！？ 燃え尽きる…！"],
    ["単語帳の 最初のページだけ ボロボロだ…！"],
    ["似た綴りで 意味が違う…！ desert と dessert…！"],
    ["語源で覚えろと 言われても もう限界だ…！"],
  ],
  // 天空の塔: リスニング・会話の悩み
  tower: [
    ["リスニングが 速すぎる…！ 一言も 聞き取れない…！"],
    ["ネイティブの雑談…！ 冗談で笑うところが 分からない…！"],
    ["話そうとすると 頭が 真っ白になる…！"],
    ["単語と単語が くっついて 聞こえる…！ 「wanna」って何だ…！"],
    ["聞き返すのが 怖くて 分かったフリをしてしまう…！"],
    ["教科書は 読めるのに 口からは 出てこない…！"],
    ["電話の英語…！ 顔が見えないと 余計に 分からない…！"],
    ["相づちすら 英語だと 変な間になる…！"],
    ["自分の発音が 通じず 3回 聞き返された…！"],
    ["頭の中で 和訳している間に 会話が 先に進む…！"],
  ],
  // 魔王城: 積年の絶望
  castle: [
    ["何年 勉強しても 話せるように ならない…！"],
    ["才能だと 言われて 心が 折れた…！"],
    ["「英語くらい できて当然」…！ その一言が 一番 つらい…！"],
    ["高い教材を 買っては 途中でやめる…！ 何度も…！"],
    ["若い頃に やっておけばと 後悔ばかりだ…！"],
    ["まわりは できるのに 自分だけ 置いていかれる…！"],
    ["結局 帰る国の 言葉すら まともに使えず…！"],
    ["努力の 方向が 間違っていたのか…！ もう 分からない…！"],
    ["「いつか話せる」…！ その いつかは 来なかった…！"],
    ["…誰か この苦しみから 解き放って くれ…！"],
  ],
};
AREAS.dungeon = {
  ...
  npcs: [WAIL_NPC(11, 9, "dungeon")], decor: [],
...
AREAS.tower = {
  id: "tower", tower: true, indoor: true, encounter: true, zone: "tower", name: "天空の塔",
  map: TOWER_MAP, cols: 13, rows: 13,
  npcs: [WAIL_NPC(10, 11, "tower")], decor: [],
...
AREAS.castle = {
  ...
  npcs: [WAIL_NPC(10, 10, "castle")], decor: [],
...
AREAS.dungeon2 = {
  ...
  npcs: [WAIL_NPC(11, 9, "dungeon2")], decor: [],
  doors: [{ tx: 7, ty: 10, to: "field" }],
};
// 氷の洞窟の敵
...
let endingSaved = false;     // エンディングで町セーブに成功したか(クリア画面の表示用)
...
const BOSS = { name: "魔王", hp: 60, atk: 9, exp: 0, color: "#c0392b", boss: true };
const TRUE_BOSS = { name: "真魔王", hp: 100, atk: 12, exp: 0, color: "#7a1f2b", boss: true }; // 魔王撃破後の第2形態
...
function saveGame() {
  ...
  const data = {
    ...
    wordCorrect: { ...wordCorrect }, npcAffection: { ...npcAffection }, permaBuffs: [...permaBuffs], metNPCs: [...metNPCs], ownedHome, kotoha: { ...kotoha },
...
function loadGame() {
  ...
  permaBuffs = data.permaBuffs || []; // 永続バフはステータス再計算より前に復元
  recomputeStats();
...
function startGame(level) {
  ...
  wordCorrect = {}; zone = ""; npcAffection = {}; permaBuffs = []; affectionRecent = {}; metNPCs = new Set(); ownedHome = null; refreshHome();
...
function drawEquip() {
  ...
  // 好感度100で得た永続バフの一覧(画面下部)
  if (permaBuffs.length) {
    const pb = permaTotals();
    const eff = [];
    if (pb.atk) eff.push(`攻+${pb.atk}`);
    if (pb.def) eff.push(`防+${pb.def}`);
    if (pb.hp) eff.push(`HP+${pb.hp}`);
    if (pb.expPct) eff.push(`経験値+${pb.expPct}%`);
    if (pb.goldPct) eff.push(`G+${pb.goldPct}%`);
    ctx.fillStyle = "#9fe0c0"; ctx.font = "11px 'MS Gothic', monospace";
    ctx.fillText(`🏅 永続バフ ${permaBuffs.length}件： ${eff.join(" ")}`, W / 2, H - 10);
  }
}
...
function interactNPC(n) {
  ...
  if (n.wail) { talkWail(n.wailArea); return; }                // 嘆きの亡霊: 英語学習の苦悩をランダムに叫ぶ(AIなし・エリア別)
...
function grantSideReward(q) {
  ...
  const gotGold = addGold(q.gold); // 永続バフ(ゴールド+%)を反映
  const ups = addGuildPoints(q.gp);
  const lines = [`依頼『${q.title_ja}』達成！ ＋${gotGold}G ＋GP${q.gp}`];
...
// 永続バフのゴールド%を反映して所持金に加算(返り値=実際の増加額)
function addGold(n) {
...
window.npcAffectionReply = (id, name, d, userText) => {
  ...
  for (const t of [50, 100]) if (before < t && aff >= t) lines.push(...affectionMilestone(id, short, t));
  return { lines };
};
...
function talkWail(area) {
  for (const k in keys) keys[k] = false;
  const pool = WAIL_LINES[area] || WAIL_LINES.dungeon;
  const line = pool[Math.floor(rnd() * pool.length)];
  cutsceneDraw = drawArea;
  playCutscene([{ who: "嘆きの亡霊", lines: line }], () => { cutsceneDraw = null; messageSpeaker = null; state = STATE.TOWN; });
}
...
function enterCastle() {
  ...
  state = STATE.TOWN;
  resetEncounter();
  if (quest && quest.stage === 27) quest.stage = 28; // 魔王城に到達→目的「魔王を倒して魂を解放する」
  playTownCutscene([{
    who: "コトハ",
    lines: ["ここが魔王城…！ 中の敵は『英文法』で試してくるよ。", "奥の玉座に魔王が、その後ろには元の世界へ帰るゲートがあるはず。", "魔王を倒して、囚われた魂を解放しよう！ でぐち(下の扉)から外に戻れるよ。"],
  }]);
}
...
function onArrive() {
  ...
  if (state === STATE.TOWN) {
    ...
    if (curArea.encounter) {
      if (curArea.castle && tileAtArea(player.tx, player.ty) === "B") { startMaouEncounter(); return; } // 玉座=魔王に対面
      stepsToEncounter--;
...
function startBattle(isBoss, bossDef, bossKind) {
  ...
  const src = isBoss ? (bossDef || BOSS) : list[Math.floor(rnd() * range)];
  battle = {
    isBoss, bossKind: isBoss ? (bossKind || "maou") : null,
...
function gainExp(amount) {
  amount = Math.round(amount * (1 + permaTotals().expPct / 100)); // 永続バフ(経験値+%)
  player.exp += amount;
  const leveled = [];
...
// 玉座で魔王に対面 → 魔王のセリフ → 通常(英文法)バトル開始
function startMaouEncounter() {
  for (const k in keys) keys[k] = false;
  cutsceneDraw = drawArea;
  playCutscene([
    { who: "魔王", lines: ["つらい……。"] },
...
function afterShinMaouDefeated(leveled) {
  ...
  playCutscene([
    ...
    { who: null, lines: ["…その衝撃で、玉座の奥のゲートが", "音を立てて 砕け散った！"] },
    { who: "コトハ", lines: ["ゲートが…壊れちゃった。", `${player.name}、元の世界へ帰る道が……。`] },
    { who: player.name, lines: ["……それでも。 いつか また帰れる日まで、", "英語を 学び続けよう。"] },
    { who: "コトハ", lines: ["うん。 私が ずっと相棒だよ。", "——さあ、新しい一歩を 踏み出そう！"] },
    { who: null, lines: ["＝＝＝ プレイヤーの英語学習は つづく…… ＝＝＝"] },
  ], () => {
    cutsceneDraw = null; messageSpeaker = null;
    if (quest) quest.cleared = true;
    // クリア記録を町でセーブ(つづきからは町で再開)
    curArea = AREAS.town; zone = "";
    player.tx = TOWN_START.tx; player.ty = TOWN_START.ty;
    player.px = player.tx * TILE; player.py = player.ty * TILE;
    player.dir = "down"; player.moving = false;
    player.hp = player.maxhp;
    savedOverworld = { tx: 4, ty: 5 }; // 町を出たときのフィールド位置(町の隣)
    state = STATE.TOWN;
    endingSaved = saveGame(); // 町でセーブ(state=TOWNなので canSave)
    state = STATE.CLEAR;
  });
}
...
function render() {
  ...
  switch (state) {
    ...
    case STATE.CLEAR: drawEnding(); break;
  }
...
// 魔王城の玉座の奥にある「帰還のゲート」(元の世界へ通じる渦巻く門)
function drawReturnGate(px, py) {
  const cx = px + 16, cy = py + 4 + Math.sin(gameTime / 400) * 1.5;
  ctx.save();
  ctx.fillStyle = "rgba(120,224,210,0.18)"; ctx.beginPath(); ctx.ellipse(cx, cy, 15, 20, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "rgba(90,180,220,0.35)"; ctx.beginPath(); ctx.ellipse(cx, cy, 10, 15, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "rgba(200,240,255,0.55)"; ctx.beginPath(); ctx.ellipse(cx, cy, 5, 9, 0, 0, Math.PI * 2); ctx.fill();
  ctx.fillStyle = "#eafeff"; ctx.fillRect(cx + 9, cy - 12, 2, 2); ctx.fillRect(cx - 11, cy + 6, 2, 2); // きらめき
  ctx.restore();
}
...
function drawEnding() {
  ...
  ctx.textAlign = "center";
  ctx.fillStyle = "#7be0d2"; ctx.font = "bold 26px 'MS Gothic', monospace";
  ctx.fillText("〜 END 〜", W / 2, 110);
  ctx.fillStyle = "#fff"; ctx.font = "15px 'MS Gothic', monospace";
  const lines = [
    "魔王＝帰れなかった転生者たちの魂は",
    "解き放たれた。",
    "",
  ...
  for (let i = 0; i < lines.length; i++) ctx.fillText(lines[i], W / 2, 160 + i * 26);
  ctx.fillStyle = "#ffd24a"; ctx.font = "14px 'MS Gothic', monospace";
  ctx.fillText(`Lv ${player.level} ／ たおした数 ${player.wins}`, W / 2, 160 + lines.length * 26 + 14);
  ctx.fillStyle = "#9fe0c0"; ctx.font = "12px 'MS Gothic', monospace";
  ctx.fillText(endingSaved ? "冒険を記録しました（つづきは町から）" : "", W / 2, H - 44);
  ctx.fillStyle = "#8aa"; ctx.font = "13px 'MS Gothic', monospace";
  ctx.fillText("Enter・タップでタイトルへ", W / 2, H - 24);
}
