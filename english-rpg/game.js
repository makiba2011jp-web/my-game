...
let floristQuest = null; // 進行中の花屋フック(口調を毎ターン更新するため保持)
// 好感度に応じたリリィの口調(システムプロンプトに付与)
function floristToneText(aff) {
  if (aff >= 80) {
    return "RELATIONSHIP: You and this traveler are very close, dear friends (affection 80+/100). Speak warmly and affectionately, openly delighted to see them, playful and relaxed, using their name-less but fond, teasing closeness. Share little personal feelings.";
  }
  if (aff >= 50) {
    return "RELATIONSHIP: You and this traveler are good friends now (affection 50+/100). Speak cheerfully and warmly, clearly glad to talk with them, more open and friendly than with a stranger.";
  }
  if (aff >= 20) {
    return "RELATIONSHIP: You are starting to get along with this traveler (affection 20+/100). Be friendly and a bit more relaxed and open than at first, though still gentle.";
  }
  return "RELATIONSHIP: You have only just met this traveler (low affection). Be polite, gentle and a little shy, warming up gradually as the conversation goes well.";
}
function talkFlorist(n) {
  for (const k in keys) keys[k] = false;
  if (!Chat.aiReady()) { talkToNPC(n); return; }
  floristQuest = {
    persist: true, // 毎ターン判定するフック(quest_flagで消さない)
    tone: floristToneText(floristAffection),
    note: "set \"quest_flag\" to true ONLY when the traveler's latest message is clearly advanced or fluent English (rich vocabulary, correct and complex grammar, natural phrasing). For simple or broken English, set it to false.",
    intro: `コトハ「花屋のリリィさんだよ。きれいな英語で話すほど仲良くなれるみたい！（好感度 ${floristAffection}/100）」`,
    onReply: (d, t) => floristOnReply(d, t),
  };
  Chat.setQuest(floristQuest);
  Chat.open(n, toeicLevel, () => { floristQuest = null; });
}
// 毎ターン、英語の質に応じて好感度を上げる(段階制)
function floristOnReply(d, userText) {
  if (floristAffection >= 100) return;
  const natural = !!(d && d.correction && d.correction.natural);
  const advanced = d && d.quest_flag === true;            // 高度・流暢な英語
  const words = String(userText || "").trim().split(/\s+/).filter(Boolean).length;
  let up = 0;
  if (advanced && words >= 8) up = 5;        // 高度かつ長く流暢な英文
  else if (advanced) up = 3;                 // 高度・流暢な英文
  else if (natural) up = 1;                  // 自然で正しい英文
  if (up <= 0) return;
  const before = floristAffection;
  floristAffection = Math.min(100, floristAffection + up);
  // 次のターンの口調に新しい好感度を反映(同じフックオブジェクトを更新)
  if (floristQuest) floristQuest.tone = floristToneText(floristAffection);
  if (window.Chat && Chat.info) Chat.info(`💗 リリィの好感度 +${up}（${floristAffection}/100）`);
  // 節目のセリフ
  const passed = (t) => before < t && floristAffection >= t;
  if (passed(100)) Chat.info("コトハ「リリィと大の仲良しになった！ 花束をくれたよ🌹」");
  else if (passed(70)) Chat.info("コトハ「リリィ、すっかり心を開いてる感じ！」");
  else if (passed(40)) Chat.info("コトハ「リリィ、うれしそうだね！」");
}
